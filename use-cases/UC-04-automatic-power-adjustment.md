...
- `+1 => -3`, `+2 => -6`, `>=+3 => -10`
- `-1 => +3`, `-2 => +6`, `<=-3 => +10`
- Convert rate to per-loop adjustment using the 5-second loop:
...
## Worked Example (5 second loop)
- If `delta <= -3`, mapped rate is `+10 W/min`.
- At a 5-second loop, each tick applies about `+0.83 W` on average.
- Over 12 ticks (60 seconds), total increase is `+10 W` (not `+10 W` per loop).
